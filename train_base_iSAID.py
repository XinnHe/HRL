...
from model.util import BaseNet
from data.dataset import FSSDataset
from util import config
from util.util import AverageMeter, intersectionAndUnionGPU, get_model_para_number, setup_seed, get_logger, \
    get_save_path,get_save_base_path, \
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='BaseNet', help='')  #
    parser.add_argument('--split', type=int, default=0, help='')  #
    parser.add_argument('--dataset', type=str, default='isaid', help='')  #iSAID  LoveDA  DLRSD
    parser.add_argument('--backbone', type=str, default='resnet50', help='')  # vgg  resnet101
...
def get_model(args):
    model = eval(args.arch).OneModel(args)  # PSPNet
    for n, p in model.named_parameters():
        if "transformer" not in n and "layer" not in n and p.requires_grad:
            print(n)
    param_dicts = [
        {"params": [p for n, p in model.named_parameters() if "layer0" in n
                    and p.requires_grad],'lr': args.base_lr},
        {"params": [p for n, p in model.named_parameters() if "layer1" in n
                    and p.requires_grad], 'lr': args.base_lr},
        ...
        {"params": [p for n, p in model.named_parameters() if "layer4" in n
                    and p.requires_grad], 'lr': args.base_lr},
        {"params": [p for n, p in model.named_parameters() if "residual_block" in n
                    and p.requires_grad], 'lr': args.base_lr*10},  #bu *10
        {"params": [p for n, p in model.named_parameters() if "ppm" in n
                    and p.requires_grad], 'lr': args.base_lr * 10},  # bu *10
        {"params": [p for n, p in model.named_parameters() if "cls" in n
                    and p.requires_grad], 'lr': args.base_lr * 10},  # bu *10
...
def main():
    ...
    # ----------------------  DATASET  ----------------------
    args.datapath='E:/5-code/data/remote_sensing/iSAID_patches'
    args.img_size=256
    ...
    train_data,train_loader = FSSDataset.build_dataloader(benchmark=args.dataset, bsz=args.batch_size,
                                                 nworker=args.workers, fold=args.split,
                                                 mode='trn', aug=args.aug,classtype='base')
    val_data,val_loader = FSSDataset.build_dataloader(benchmark=args.dataset, bsz=args.batch_size,
                                                 nworker=args.workers, fold=args.split,
                                                 mode='val',classtype='base')
    ...
    model, optimizer,transformer_optimizer = get_model(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # ----------------------  TRAIN  ----------------------
        train(train_loader, val_loader, model, optimizer, transformer_optimizer,epoch, scaler)
        # save model for <resuming>
        if ((epoch + 1) % args.save_freq == 0) and main_process():  # save_freq=5
        ...
        if args.evaluate and (epoch + 1) % args.val_freq == 0:  # val_freq=1
            ...
            if mIoU > best_miou:
                ...
                with open(grow_name, 'a') as f:
                    f.write(
                        'Best_epoch:{} , Best_miou:{:.4f} , fbIou:{:.4f} , recall:{:.4f}, precision:{:.4f}, \n'.format(
                            epoch, best_miou, fbIou, recall, precision))
    ...
    print('\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  Final Best Result   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
    print(
        args.arch + '\t Group:{} \t Best_mIoU:{:.4f} \t Best_FBIoU:{:.4f} \t Best_step:{}'.format(args.split, best_miou,
                                                                                                  best_FBiou,
                                                                                                  best_epoch))
    print('>' * 80)
...
def train(train_loader, val_loader, model, optimizer,transformer_optimizer, epoch, scaler):
    ...
    for i, batch in enumerate(train_loader):
        ...
        cur_lr4 = optimizer.param_groups[4]['lr']
        cur_lr5=optimizer.param_groups[5]['lr']
        #cur_trans_lr1=transformer_optimizer.param_groups[1]['lr']# base_lrs
        ...
        if current_iter % 50 == 0 and main_process():
            logger.info(' ' * len(current_characters[0]) * 3 + ' ' * 10 + 'Base LR: {:.8f}, Curr LR0: {:.8f},Curr LR4: {:.8f},Curr LR5: {:.8f}'.format(
                args.base_lr, cur_lr0,cur_lr4,cur_lr5))
            logger.info(current_characters[0] * 3 + ' ' * 5 + '{}_{}_{}_split{} Pretrain: {} GPU_id: {}'.format(args.arch, \
                                                                                                          args.dataset,
                                                                                                          args.backbone,
                                                                                                          args.split,
                                                                                                          args.pretrain,
                                                                                                          current_GPU) + ' ' * 5 +
                  current_characters[1] * 3)
        ...
        output, main_loss, aux_loss_1, aux_loss_2 = model(x=input, y=target,padding_mask = None)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():  # print_freq=10
            logger.info('Epoch: [{}/{}][{}/{}] '
        ...
        # -----------------------  SubEpoch VAL  -----------------------
        if args.evaluate and args.SubEpoch_val and (args.epochs <= 100 and (epoch + 1) % args.val_freq == 0) and (
                i == round(len(train_loader) / 2)):  # max_epoch<=100时进行half_epoch Val
            _, fbIou, _, _, mIoU, _, recall, precision = validate(val_loader, model)
            model.train()
            val_num += 1
            # save model for <testing> and <fine-tuning>
...
def validate(val_loader, model):
    ...
    test_num = min(len(val_loader.dataset.img_metadata), 1000)  # 20000 .img_metadata
    ...
    for e in range(10):
        for i, batch in enumerate(val_loader):
            ...
            # with autocast():
            with torch.no_grad():
                output = model(x=input, y=target,padding_mask=None)
            ...
            precision = np.mean(intersection_meter.val[1:] / (
                        union_meter.val[1:] - target_meter.val[1:] + intersection_meter.val[1:] + 1e-10))
    ...
    for i in range(2):#aa=val_loader.dataset.class_name[val_loader.dataset.class_ids[0]]
        logger.info('Class_{} Result: iou/accuracy {:.4f}/{:.4f}.'.format(i, iou_class[i], accuracy_class[i]))
